...
namespace CSHARP_MAIN_CRAWLER
{
    public class Connector
    {
        ...
        private void init()
        {
            ...
            user = "root";
            password = "forageme";
            string str = "server=localhost;database=db_forager;userid=root;password=forageme;";
        ...
        public bool openConnection()
        {
            ...
            {
                Console.WriteLine("Failed OPEN Connection to DB: ");
                return false;
            }
        ...
        public bool closeConnection()
        {
            ...
            {
                Console.WriteLine("Failed CLOSE Connection to DB: ");
                return false;
            }
        }
        ...
        public void do_create_scan_tables(int scan_id)
        {
            ...
            query += "`url` varchar(1000) DEFAULT NULL,";
            query += "`domain` varchar(1000) DEFAULT NULL,";
            query += "`link` varchar(1000) DEFAULT NULL,";
            query += "`source` varchar(1000) DEFAULT NULL,";
            query += "`url_type` int(11) DEFAULT NULL,";
            query += "`status_code` int(11) DEFAULT NULL,";
            query += "`status_code_type` varchar(1000) DEFAULT NULL,";
            query += "`state` tinyint(1) DEFAULT NULL,";
            ...
            query += "`url_id` int(11) DEFAULT NULL,";
            query += "`dest_id` int(11) DEFAULT NULL";
        ...
        public void do_scanner_stop_updates()
        {
            ...
            /*
            UPDATE `scan` 
            SET pages_scanned = (SELECT COUNT(*) FROM `url4` WHERE url_type = 1) , number_errors = (SELECT COUNT(*) FROM `url4` WHERE state = 0), is_running = 0 
            WHERE scan_id = 4
            */

            string update_query = "UPDATE `scan` ";
            update_query += "SET pages_scanned = (SELECT COUNT(*) FROM `" + url_table + "` WHERE url_type = 1) , number_errors = (SELECT COUNT(*) FROM `" + url_table + "` WHERE state = 0 AND url NOT LIKE '%gilfind.spsu%'), is_running = 0 ";
        ...
        public int do_check_started()
        {
            ...
            if (this.openConnection() == true)
            {
                ...
                if (reader.Read())
                {
                    int scan_id = reader.GetInt32("scan_id");
                    reader.Close();
                    this.closeConnection();
                    return scan_id;
                }
